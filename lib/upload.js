...
export async function saveUploadedImage(file, subdir) {
  ...
  const dir = path.join(/* turbopackIgnore: true */ UPLOADS_BASE, subdir);
...
export async function deleteUploadedImage(publicPath) {
  ...
  const filePath = path.join(/* turbopackIgnore: true */ UPLOADS_BASE, publicPath.slice("/uploads/".length));
  try {
